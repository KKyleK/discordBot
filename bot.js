...
client.on("messageCreate", msg => {
    if(msg.content == "=j") {
        ...
        if(picking){ 
            console.log("Players currently being picked, wait.");
        }
            ...
            console.log(msg.member.displayName + " Is already in the queue.")
        }
    }

    if(games[current_game].queue == 10){   //queue pop
        ...
        console.log("Captain 1: " + captain1 + " Captain 2: "+ captain2);
    ...
    //Message is: @!id
    if(message[0] != null && message[0]== "=p" && message.length == 3 && msg.displayName == current_captain) {
        
        current_captain = next_captain; 
        if(next_captain == captain1){ //Get next captain.
            next_captain = captian2;
        }
        else{
